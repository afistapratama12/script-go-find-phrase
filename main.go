...
func main() {
	...
	keyETHER := []string{
		...
		"SJAJ2GC8Z93Y4HIPWHKJE4ZFMAWARC8EQF",
	}
	...
	keyBSC := []string{
		...
		"J7MVJ8Z48RNPAFN6439FBE15IFNYAYJRYA",
	}
	...
	svc := NewService(db, words, keyETHER, keyBSC)
...
func (s *service) ProcessGetPhrase() {
	wg := &sync.WaitGroup{}
	mu := &sync.Mutex{}
...
func (s *service) CallAPI(path string) ([]string, error) {
	...
	if response.Status != "1" && response.Message != "OK" {
		return nil, errors.New("error response from api, not 1 OK")
	}

	var listAddressBalance []string

	for _, balance := range response.Result {
		if balance.Balance != "0" && balance.Balance != "0x0" && balance.Balance != "" {
			listAddressBalance = append(listAddressBalance, balance.Account)
		}
	}
...
func (s *service) GetAddresses(mnemonic string) []string { // expect get 2 address
	...
	path = hdwallet.MustParseDerivationPath("m/0/0")
	account, _ := wallet.Derive(path, false)

	res = append(res, account.Address.Hex())

	// max 2 address
	path = hdwallet.MustParseDerivationPath("m/44'/60'/0'/0/0")
	account2, _ := wallet.Derive(path, false)

	res = append(res, account2.Address.Hex())
